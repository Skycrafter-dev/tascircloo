...
void b2ContactSolver::InitializeVelocityConstraints()
{
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			b2Vec2 tangent = b2Cross(vc->normal, 1.0);
...
void b2ContactSolver::WarmStart()
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		float32 tangentX = 1.0 * normal.y;
		float32 tangentY = -1.0 * normal.x;

		for (int32 j = 0; j < pointCount; ++j)
		{
			b2VelocityConstraintPoint* vcp = vc->points + j;
			float32 Px = vcp->normalImpulse * normal.x + vcp->tangentImpulse * tangentX;
			float32 Py = vcp->normalImpulse * normal.y + vcp->tangentImpulse * tangentY;
...
void b2ContactSolver::SolveVelocityConstraints()
{
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			float32 dvx = vB.x + (-wB * vcp->rB.y) - vA.x - (-wA * vcp->rA.y);
			float32 dvy = vB.y + (wB * vcp->rB.x) - vA.y - (wA * vcp->rA.x);

			float32 vt = (dvx * tangentX + dvy * tangentY) - vc->tangentSpeed;
			...
			float32 Px = lambda * tangentX;
			float32 Py = lambda * tangentY;
			vA.x -= mA * Px;
			vA.y -= mA * Py;
			wA -= iA * (vcp->rA.x * Py - vcp->rA.y * Px);
			vB.x += mB * Px;
		...
		if (pointCount == 1 || g_blockSolve == false)
		{
			for (int32 i = 0; i < pointCount; ++i)
			{
				...
				float32 vn = dvx * normal.x + dvy * normal.y;
				...
				float32 Px = lambda * normal.x;
				float32 Py = lambda * normal.y;
				vA.x -= mA * Px;
				vA.y -= mA * Py;
				wA -= iA * (vcp->rA.x * Py - vcp->rA.y * Px);
				vB.x += mB * Px;
